...
export default function LoginPage() {
  ...
  const onFacebookSuccess = async (response: SuccessResponse) => {
    try {
      ...
      if (!fbData.success) {
        setMessageFB({
          text: fbData.message || "Facebook login failed",
          severity: "error",
        });
        return;
      }
      ...
      if (!signupRes.ok && !signupData.message.includes("already exists")) {
        setMessageFB({
          text: signupData.message || "Signup failed",
          severity: "error",
        });
        return;
      }
        ...
        <div className="bg-card -m-px rounded-[calc(var(--radius)+.125rem)] border p-8 pb-6">
          <div className="text-center">
            ...
            <h1 className="mb-1 mt-4 text-xl font-semibold">
              Log In to JSCGO: San Isidro
            </h1>
            <p className="text-sm">
              Fill up the form below to access your account.
            </p>
          </div>
          ...
          <div className="mt-6 space-y-6">
            ...
            {messageFB && (
              <div
                className={`text-center mt-2 ${messageFB.severity === "error" ? "text-red-500" : "text-green-500"}`}
              >
                {messageFB.text}
              </div>
            )}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Button type="button" variant="outline">
              ...
              >
                <path
                  d="M474.077 236.966A236.966 236.966 0 1 1 0.145 236.966A236.966 236.966 0 1 1 474.077 236.966Z"
                  fill="#3A5A98"
                />
                <path
                  d="M404.742,69.754c92.541,92.541,92.545,242.586-0.004,335.134c-92.545,92.541-242.593,92.541-335.134,0L404.742,69.754z"
                  fill="#345387"
                />
                <path
                  d="M472.543,263.656L301.129,92.238l-88.998,88.998l5.302,5.302l-50.671,50.667l41.474,41.474l-5.455,5.452l44.901,44.901l-51.764,51.764l88.429,88.429C384.065,449.045,461.037,366.255,472.543,263.656z"
                  fill="#2E4D72"
                />
                <path
